...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "set-watching") message.type = "set-enabled";
  if (message.type === "set-enabled") {
    const saveSettings = () => chrome.storage.local
      .set({
    ...
    if (message.enabled) {
      activeLicense().then(ok => ok ? saveSettings() : sendResponse({ ok: false, message: "An active license is required." }));
    } else {
      saveSettings();
    }
    return true;
  ...
  if (message.type === "save-settings") {
    (async () => {
      const current = await chrome.storage.local.get(["enabled", "watching"]);
      const enabled = Boolean(current.enabled || current.watching);
      if (enabled && !await activeLicense()) {
        sendResponse({ ok: false, message: "An active license is required." });
        return;
      }
      ...
      await setAlarm(enabled, message.intervalMinutes);
      sendResponse({ ok: true });
